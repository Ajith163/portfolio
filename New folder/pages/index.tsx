...
export default function Home() {
    ...
    if (!IsBlackListEmpty) {
      if (userData) {
        ...
        const blacklistCountries = process.env.NEXT_PUBLIC_BLACKLIST_COUNTRIES
          ? process.env.NEXT_PUBLIC_BLACKLIST_COUNTRIES.split(',').map(c => c.trim())
          : [];
  ...
  const meta = {
    title: "Ajith Kumar - Frontend Software Engineer",
      ...
      {!isBlackListed ? (
        <div className="relative snap-mandatory min-h-screen bg-AAprimary w-full ">
          ...
          {context.sharedState.finishedLoading ? (
            <Footer githubUrl={"https://github.com/Ajith163/my-website"} hideSocialsInDesktop={true} />
          ) : (
            <></>
          )}
